...
const productSchema = new mongoose.Schema({
    name: String, hindiName: String, price: Number, stock: { type: Number, default: 100 }, unit: { type: String, default: 'kg' }
});
const Product = mongoose.model('Product', productSchema);

// Customer Model for Step 2
...
// Customer Login/Address Fetch
app.post('/api/customer/get-address', async (req, res) => {
    try {
        const customer = await Customer.findOne({ mobile: req.body.mobile });
        res.json({ address: customer ? customer.address : null });
    } catch (err) { res.status(500).json({ error: "Server Error" }); }
});

// Order save karte waqt address bhi save karein
app.post('/api/customer/save', async (req, res) => {
    try {
        const { mobile, address } = req.body;
        await Customer.findOneAndUpdate({ mobile }, { address, lastOrder: new Date() }, { upsert: true });
