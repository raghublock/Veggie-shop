const express = require('express');
const mongoose = require('mongoose');
const path = require('path');
require('dotenv').config();

const app = express();
app.use(express.json());
app.use(express.urlencoded({ extended: true }));

app.set('view engine', 'ejs');
app.set('views', path.join(__dirname, 'views'));

mongoose.connect(process.env.MONGODB_URI)
    .then(() => console.log("Bikaner DB Connected"))
    .catch(err => console.error("DB Connection Fail:", err));

// MODELS
const Product = mongoose.model('Product', new mongoose.Schema({
    name: String, hindiName: String, price: Number, stock: { type: Number, default: 100 }, unit: { type: String, default: 'kg' }, isFree: { type: Boolean, default: false }
}));

const Customer = mongoose.model('Customer', new mongoose.Schema({
    mobile: { type: String, unique: true }, address: String, lastOrder: { type: Date, default: Date.now }
}));

const Settings = mongoose.model('Settings', new mongoose.Schema({
    mandiMessage: { type: String, default: "Bikaner Mandi ke aaj ke taaza bhav!" }
}));

// ROUTES
app.get('/', (req, res) => res.render('index'));
app.get('/admin', (req, res) => {
    if (req.query.pass === 'bhati@123') res.render('admin');
    else res.send("Unauthorized! URL mein ?pass=bhati@123 lagayein.");
});

// APIs
app.get('/api/settings', async (req, res) => {
    let s = await Settings.findOne();
    if (!s) s = await Settings.create({});
    res.json(s);
});

app.post('/api/settings/update', async (req, res) => {
    await Settings.findOneAndUpdate({}, { mandiMessage: req.body.message }, { upsert: true });
    res.json({ success: true });
});

app.get('/api/products', async (req, res) => res.json(await Product.find({})));

app.post('/api/products/add', async (req, res) => {
    const p = new Product(req.body);
    await p.save();
    res.json({ success: true });
});

app.post('/api/products/update/:id', async (req, res) => {
    await Product.findByIdAndUpdate(req.params.id, req.body);
    res.json({ success: true });
});

app.delete('/api/products/delete/:id', async (req, res) => {
    await Product.findByIdAndDelete(req.params.id);
    res.json({ success: true });
});

app.post('/api/customer/get-address', async (req, res) => {
    const c = await Customer.findOne({ mobile: req.body.mobile });
    res.json({ address: c ? c.address : null });
});

app.post('/api/customer/save', async (req, res) => {
    await Customer.findOneAndUpdate({ mobile: req.body.mobile }, { address: req.body.address, lastOrder: new Date() }, { upsert: true });
    res.json({ success: true });
});

app.post('/api/user/check-radius', (req, res) => res.json({ inRadius: true }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`Server Live!`));
